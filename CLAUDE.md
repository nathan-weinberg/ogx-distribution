...
1. **`build/build.py`** (hook: `pkg-gen`, always runs) — creates a temp venv, installs OGX, runs `ogx stack list-deps` against `build/build.yaml` to discover Python dependencies, then generates:
   ...
   - `distribution/requirements-lock.txt` — locked pip requirements with hashes, including OpenTelemetry instrumentation packages
...
- `distribution/requirements-lock.txt` — generated by `build/build.py`
