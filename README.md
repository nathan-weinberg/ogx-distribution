...
The `distribution/config.yaml` and `distribution/requirements-lock.txt` files are auto-generated by `build/build.py`. To regenerate them:
...
- Execute `./build/build.py` to generate `distribution/config.yaml` and `distribution/requirements-lock.txt`
- Execute `./build/gen_distro_docs.py` to regenerate `distribution/README.md`

> [!WARNING]
> Do not edit `distribution/config.yaml`, `distribution/requirements-lock.txt`, or `distribution/README.md` manually — they are regenerated on every commit.
